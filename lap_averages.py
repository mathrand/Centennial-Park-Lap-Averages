...
def lap_to_sec(laptime):
    seconds = (
        (3600 * int(laptime.split(":")[0]))
        + (60 * int(laptime.split(":")[1]))
        + int(laptime.split(":")[2])
    )
    return seconds
...
for f in rides_dict:
    if int(f[1]) == int(last_activity_id):
        ...
        if len(fastest_lap_per_activity) > 0:
            ...
            fastest_lap_all_time.append(
                [fastest_lap_per_activity[1], activity.name, other_athletes]
            )
            average_laps_all_time.append(
                [(average_laps / lap_count), activity.name, other_athletes]
            )
        print("===========================")
